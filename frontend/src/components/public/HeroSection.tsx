...
export default function HeroSection() {
  ...
  return (
    ...
    >
      ...
      {/* TOP HEADER: BRAND, CAROUSEL DOTS, COMPACT MOBILE CTAS */}
      <header className="relative z-30 w-full px-4 sm:px-12 lg:px-16 pt-5 sm:pt-7 pb-4 flex items-center justify-between gap-2">
        
        {/* LEFT: HOTEL BOUTIQUE BRAND & SOCIAL ICONS */}
        <div className="flex items-center gap-4 sm:gap-6">
          <span className="font-semibold text-[11px] sm:text-sm tracking-[0.24em] sm:tracking-[0.28em] uppercase text-[#F5F5F1] cursor-default whitespace-nowrap">
            HOTEL BOUTIQUE
          </span>

          {/* ACTUAL SOCIAL MEDIA ICONS (HOVER COLOR SHIFT TO #8BBB92) */}
          <div className="hidden sm:flex items-center gap-3.5 pl-4 border-l border-white/20">
            ...
            <a
              ...
              className="text-[#F5F5F1]/75 hover:text-[#8BBB92] hover:scale-110 transition-all duration-200"
            >
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 bg-[#092328]/80 backdrop-blur-md px-2.5 sm:px-3.5 py-1.5 rounded-full border border-white/10 shrink-0">
          {SLIDES.map((_, dotIdx) => (
            <button
              key={dotIdx}
              ...
              className={`rounded-full transition-all duration-300 ${
                dotIdx === currentSlide
                  ? "w-4 sm:w-6 h-1 sm:h-1.5 bg-[#8BBB92]"
                  : "w-1 sm:w-1.5 h-1 sm:h-1.5 bg-[#F5F5F1]/30 hover:bg-[#8BBB92]"
              }`}
            />
          ))}
        </div>

        {/* RIGHT: STRATEGIC COMPACT NON-WORKING CTA "GET IN TOUCH" */}
        <div className="flex items-center">
          <a
            href="#"
            onClick={(e) => e.preventDefault()}
            className="text-[10px] sm:text-xs font-semibold tracking-[0.16em] sm:tracking-[0.22em] uppercase text-[#092328] bg-[#F5F5F1] hover:bg-[#12544F] hover:text-[#F5F5F1] hover:border-[#092328] transition-all duration-300 px-3 sm:px-5 py-1.5 sm:py-2.5 rounded-full border border-[#E5E5DF] shadow-md whitespace-nowrap"
          >
            GET IN TOUCH
          </a>
        </div>
