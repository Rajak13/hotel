...
export default function HeroSection() {
  ...
  return (
    <section
      className="relative w-full h-screen min-h-[640px] max-h-[1080px] bg-[#092328] text-white overflow-hidden select-none flex flex-col justify-between"
    ...
    >
          ...
          return (
            ...
            >
              <Image
                ...
                sizes="100vw"
              />
              {/* ATMOSPHERIC COLOR VIGNETTES USING EXACT PALETTE: #092328 */}
              <div className="absolute inset-0 bg-[#092328]/50" />
      ...
      <header className="relative z-30 w-full px-6 sm:px-12 lg:px-16 pt-7 pb-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          <span className="font-semibold text-xs sm:text-sm tracking-[0.28em] uppercase text-white cursor-default">
            HOTEL BOUTIQUE
          </span>

          {/* ACTUAL SOCIAL MEDIA ICONS (CLEAN VECTOR SVGS IN #8BBB92 COLOR) */}
          <div className="hidden sm:flex items-center gap-3.5 pl-4 border-l border-[#12544F]">
            {/* Instagram */}
            <a
              href="#"
              onClick={(e) => e.preventDefault()}
              aria-label="Instagram"
              className="text-[#8BBB92] hover:text-white transition-colors"
            >
        ...
        <div className="flex items-center gap-2 bg-[#092328]/80 backdrop-blur-md px-3.5 py-1.5 rounded-full border border-[#12544F]">
          {SLIDES.map((_, dotIdx) => (
            <button
              key={dotIdx}
              ...
              className={`rounded-full transition-all duration-300 ${
                dotIdx === currentSlide
                  ? "w-6 h-1.5 bg-[#8BBB92]"
                  : "w-1.5 h-1.5 bg-[#8BBB92]/40 hover:bg-[#8BBB92]/70"
              }`}
            />
          ))}
        </div>

        {/* RIGHT: STRATEGIC NON-WORKING CTA "GET IN TOUCH" */}
        <div className="flex items-center gap-4">
          <a
            href="#"
            onClick={(e) => e.preventDefault()}
            className="text-xs font-medium tracking-[0.24em] uppercase text-white hover:text-[#8BBB92] transition-colors px-4 py-2 rounded-full border border-[#12544F] bg-[#12544F]/30 hover:bg-[#12544F]/60"
          >
            GET IN TOUCH
          </a>
        </div>
      ...
      {/* LOWER HALF: MONUMENTAL TYPOGRAPHY + INTEGRATED FLOATING BOOKING BAR */}
      <div className="relative z-20 flex-1 flex flex-col items-center justify-end pb-8 sm:pb-12 text-center px-4">
        
        {/* SLIDE CAPTION IN SAGE ACCENT #8BBB92 */}
        <p className="text-xs sm:text-sm font-medium tracking-[0.3em] uppercase text-[#8BBB92] mb-2 drop-shadow-md">
          {activeSlideData.caption}
        </p>

        {/* MONUMENTAL TITLE OVER THE LANDSCAPE */}
        <h1 className="text-[15vw] sm:text-[14vw] lg:text-[13.5vw] font-normal tracking-[-0.02em] text-white uppercase leading-[0.85] font-sans drop-shadow-[0_12px_30px_rgba(0,0,0,0.8)] select-none">
          {activeSlideData.word}
        </h1>

        {/* LOCATION FOOTNOTE IN #8BBB92 */}
        <div className="flex items-center gap-3 my-4 text-[10px] sm:text-xs tracking-[0.28em] uppercase text-[#8BBB92]">
          <span>Dharan Hills</span>
          <span className="w-1 h-1 rounded-full bg-[#2A835F]" />
          <span>Koshi Province</span>
          <span className="w-1 h-1 rounded-full bg-[#8BBB92]" />
          <span>Nepal</span>
        </div>

        {/* FLOATING BOOKING BAR DOCKED IN LOWER HALF */}
